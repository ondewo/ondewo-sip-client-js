...
//
// This is an ARTEFACT test. `api/ondewo_sip_api.js` is a self-contained browser bundle: it embeds the
...
const BUNDLE_PATH = path.join(__dirname, '..', 'api', 'ondewo_sip_api.js');
 ...
 *
 * @returns {any} the `ondewo_sip_api` namespace object.
 */
function loadApiBundle() {
	...
	assert.ok(context.ondewo_sip_api, 'the bundle did not define the ondewo_sip_api global');
	return context.ondewo_sip_api;
}

runTestCase('a string field survives a binary round trip through the shipped bundle', () => {
	const api = loadApiBundle();
	assert.ok(api.SipRegisterAccountRequest, 'the bundle does not export SipRegisterAccountRequest');

	const original = new api.SipRegisterAccountRequest();
	...
	const decoded = api.SipRegisterAccountRequest.deserializeBinary(bytes);
	assert.equal(decoded.getAccountName(), value, 'the string did not survive the round trip');
});

runTestCase('the bundle carries a protobuf runtime able to read the strings it writes', () => {
	const api = loadApiBundle();
	const message = new api.SipRegisterAccountRequest();
	message.setAccountName('probe');
	...
	assert.doesNotThrow(
		() => api.SipRegisterAccountRequest.deserializeBinary(message.serializeBinary()),
