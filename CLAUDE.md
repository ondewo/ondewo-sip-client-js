...
This repo has submodules (`ondewo-proto-compiler`, `src/ondewo-sip-api`). If a submodule's working
...
```text
Release ONDEWO SIP Js Client ${ONDEWO_SIP_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO SIP Js Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO SIP Js Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`@ondewo/ondewo-sip-client-js`**, which is not always the repository name — the JS client
...
```bash
npm view @ondewo/ondewo-sip-client-js versions --json
```
